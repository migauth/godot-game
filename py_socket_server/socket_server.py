...
# Define a dictionary of lambdas keyed by opcodes
opcode_handlers = {
...
class GameSession:
    def __init__(self, host_ip, session_name, host_ws):
        ...
        self.client_ws = set()  # Set to store client WebSockets

    def add_client(self, client_ws):
        self.client_ws.add(client_ws)
...
async def handle_message(websocket, message):
    ...
    try:
        ...
        _handler = opcode_handlers.get(opcode)
        if _handler:
...
async def on_game_posted(websocket, data):
    ...
    game_session = GameSession(websocket.remote_address, game_name, websocket)
    game_sessions[game_name] = game_session
    print(f"Game session '{game_name}' posted by {websocket.remote_address}.")
...
async def on_start_confirmation(websocket):
    ...
    for game_session in game_sessions.values():
        if game_session.host_ws == websocket:
            ...
            await game_session.broadcast_message(json.dumps({"opcode": OP_START_CONFIRMATION, "message": "Start"}))
...
async def on_join_game(websocket, data):
    ...
    game_session = game_sessions.get(game_name)
    if game_session:
        game_session.add_client(websocket)
        print(f"Player {websocket.remote_address} joined game session '{game_name}'.")
        # Notify all members of the game session about the new player
        await game_session.broadcast_message(json.dumps({"opcode": OP_JOIN_GAME, "message": f"Player {websocket.remote_address} joined the game."}))
    else:
        print(f"Game session '{game_name}' not found.")
